...
class SingleFlow extends Binder {
  ...
  _handleTaskSubmit(e) {
    ...
    newTask.createdby = loggedInUser._id;
  ...
  render() {
     ...
     */
    let taskListItems = taskStore.util.getList("_flow", match.params.flowId);
    ...
    let completedTaskListItems = null; 
    if (taskListItems) {
      completedTaskListItems = taskListItems.filter(x => x.status === "approved");
      taskListItems = taskListItems.filter(x => x.status === "open");
    }
      ...
      <FlowLayout>
                ...
                <ul>
                  {taskListItems.map((task, i) =>
                    <li key={task._id + i}>
                      ...
                      <Link className="btn plain-bordered" to={`/tasks/${task._id}`}>Comment</Link>
                    </li>
                  )}
                </ul>
              </div>
        ...
        <div className="completed-task">
            <h4>Completed Task</h4>
            {
              ...
              <ul>
                {
                  completedTaskListItems.map((task, i) => 
                    <li key={i}>
                      { task.complete ? 
                        <p>{task.name}</p> : <del><p>{task.name}</p></del>
                      }
                    </li>
                  )
                } 
